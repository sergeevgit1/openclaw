  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Узлы</div>
          <div class="card-sub">Сопряжённые устройства и активные подключения.</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">Узлы не найдены.</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">Запросы на сопряжение и токены ролей.</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? "Загрузка…" : "Обновить"}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">Ожидающие</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">Сопряжённые</div>
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">Нет сопряжённых устройств.</div>
              `
            : nothing
        }
      </div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim() ? `роль: ${req.role}` : "роль: -";
  const repair = req.isRepair ? " · восстановление" : "";
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `роли: ${formatList(device.roles)}`;
    ...
    <div class="list-item" style="grid-template-columns: 1fr;">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">Токены: нет</div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">Токены</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? "отозван" : "активен";
  const scopes = `области: ${formatList(token.scopes)}`;
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  ...
  { value: "allowlist", label: "Список разрешённых" },
  { value: "full", label: "Полный" },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: "Выкл" },
  { value: "on-miss", label: "При отсутствии" },
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">Привязка узла выполнения</div>
          <div class="card-sub">
            Привязка агентов к определённому узлу при использовании <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? "Сохранение…" : "Сохранить"}
        </button>
      </div>
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                Переключите вкладку Конфигурация в режим <strong>Форма</strong> для редактирования привязок.
              </div>
            `
          : nothing
      }
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">Загрузите конфигурацию для редактирования привязок.</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? "Загрузка…" : "Загрузить конфигурацию"}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">Привязка по умолчанию</div>
                  <div class="list-sub">Используется, когда агенты не переопределяют привязку узла.</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>Узел</span>
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>Любой узел</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">Нет узлов с поддержкой system.run.</div>
                        `
                      : nothing
                  }
                </div>
...
function renderExecApprovals(state: ExecApprovalsState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">Разрешения на выполнение</div>
          <div class="card-sub">
            Список разрешений и политика для <span class="mono">exec host=gateway/node</span>.
          </div>
        </div>
        <button
          class="btn"
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">Загрузите разрешения для редактирования списков.</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? "Загрузка…" : "Загрузить разрешения"}
            </button>
          </div>`
          : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Цель</div>
          <div class="list-sub">
            Шлюз редактирует локальные разрешения; узел — выбранный узел.
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            state.target === "node"
              ? html`
                <label class="field">
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>Выберите узел</option>
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">Ни один узел ещё не поддерживает разрешения на выполнение.</div>
            `
          : nothing
      }
    </div>
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? "Режим безопасности по умолчанию." : `По умолчанию: ${defaults.security}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    По умолчанию (${defaults.security})
                  </option>`
                  : nothing
              }
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Запрос</div>
          <div class="list-sub">
            ${isDefaults ? "Политика запросов по умолчанию." : `По умолчанию: ${defaults.ask}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Запрос (резерв)</div>
          <div class="list-sub">
            ${
              isDefaults
                ? "Применяется, когда UI-запрос недоступен."
                : `По умолчанию: ${defaults.askFallback}.`
            }
          </div>
        </div>
        <div class="list-meta">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Авто-разрешение CLI навыков</div>
          <div class="list-sub">
            ${
              isDefaults
                ? "Разрешить исполняемые файлы навыков, перечисленные шлюзом."
                : autoIsDefault
                  ? `По умолчанию (${defaults.autoAllowSkills ? "вкл" : "выкл"}).`
                  : `Переопределение (${autoEffective ? "вкл" : "выкл"}).`
            }
          </div>
        </div>
        <div class="list-meta">
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">Список разрешённых</div>
        <div class="card-sub">Шаблоны glob без учёта регистра.</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">Список разрешённых пуст.</div>
            `
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : "никогда";
  const lastCommand = entry.lastUsedCommand ? clampText(entry.lastUsedCommand, 120) : null;
  const lastPath = entry.lastResolvedPath ? clampText(entry.lastResolvedPath, 120) : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : "Новый шаблон"}</div>
        <div class="list-sub">Последнее использование: ${lastUsed}</div>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? "агент по умолчанию" : "агент"} ·
          ${
            bindingValue === "__default__"
              ? `по умолчанию (${state.defaultBinding ?? "любой"})`
              : `переопределение: ${agent.binding}`
          }
        </div>
      </div>
      <div class="list-meta">
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? "сопряжён" : "не сопряжён"}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? "подключён" : "офлайн"}
          </span>
